...
class MyDataset(Dataset):#继承了Dataset子类
    ...
    def __getitem__(self, index):
        ...
        # return (input_img, age*10 + gender*5 + race)
...
def train_model(model_ft, criterion, optimizer, lr_scheduler, num_epochs=80):
    # answer.txt用作存储训练结果
    file = open('answer_b0data_utk.txt','w')
    since = time.time()
    # 最好的训练权重，初始值为预训练权重
...
if __name__ == "__main__":
    ...
    # model_ft._fc = nn.Linear(num_ftrs,11)  # 位运算
    ...
    model_ft = torch.load('UTK_Face/model/efficientnet-b0(b0_data).pth')  #可单独使用测试函数
