...
def loaddata(data_dir, batch_size, set_name, shuffle):
    ...
    #class_to_idx
    # print(image_datasets[set_name].class_to_idx)
...
def train_model(model_ft, criterion, optimizer, lr_scheduler, num_epochs=50, a = 0.1):
    # answer.txt用作存储训练结果
    file = open(os.path.join(data_dir,'answer/lfw0.7.txt'),'w')
    since = time.time()
    # 最好的训练权重，初始值为预训练权重
    ...
    for epoch in range(num_epochs):
        ...
        for data in dset_loaders['train']:
            ...
            # 由总标签到各独立任务标签
            label_age = labels%89 #lfw+共89类
            label_gender = (labels > 88)*1
            # 将labele_age转为子二分类标签（为损失函数做准备）
            levels = []
            for i in range(label_age.size()[0]):
                levels.append([1]*label_age[i] + [0]*(89 - 1 - label_age[i]))
        ...
        print('loss_age: {:.4f} loss_gender: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f}\n'.format(
        ...
        test_loaders, test_sizes = loaddata(data_dir=data_dir, batch_size=16, set_name='test', shuffle=False)
        for data in test_loaders['test']:
            inputs, labels = data
            label_age = labels%89 #lfw+共89类
            label_gender = (labels > 88)*1
            # 将labele_age转为子二分类标签（为损失函数做准备）
            ...
            test_age += torch.sum(preds_age_low * preds_age_high)
            test_gender += torch.sum(preds_gender == label_gender.data)
        #这里在终端输出每次模型的评价指标
        file.write('Acc_age: {:.4f} Acc_gender: {:.4f}\n'.format(test_age.double() / test_sizes,test_gender.double() / test_sizes))
        print('testAcc_age: {:.4f} testAcc_gender: {:.4f}\n'.format(test_age.double() / test_sizes,test_gender.double() / test_sizes))
        epoch_acc_age = test_age.double() / test_sizes
    ...
    model_out_path = save_dir + "/" + net_name + '(07).pth'
...
def test_model(model, criterion,a=0.1):
    file = open(os.path.join(data_dir,'answer/lfw0.7.txt'),'a')
    model.eval()
    # 损失值
    running_loss_age = 0.0
    running_loss_gender = 0.0
    ...
    #这里在终端输出每次模型的评价指标
    file.write('Loss_age: {:.4f}  Loss_gender: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f} mseloss_age: {:.4f}\n'.format(running_loss_age/dset_sizes,running_loss_gender/dset_sizes,
                                            running_corrects_age.double() / dset_sizes,running_corrects_gender.double() / dset_sizes, mseloss_age /dset_sizes))
    print('Loss_age: {:.4f}  Loss_gender: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f} mseloss_age: {:.4f}\n'.format(running_loss_age/dset_sizes,running_loss_gender/dset_sizes,
                                            running_corrects_age.double() / dset_sizes,running_corrects_gender.double() / dset_sizes, mseloss_age /dset_sizes))
...
if __name__ == "__main__":
    ...
    # our methods
    model_ft._fc = nn.Linear(num_ftrs, (89-1)*2)
    # 消融实验
    # model_ft._fc = nn.Linear(num_ftrs,51)
    ...
    # 调用训练函数
    best_model_wts = train_model(model_ft, criterion, optimizer, exp_lr_scheduler, num_epochs=num_epochs,a = 0.7)
    # 调用测试函数 输出模型评价指标
    print('-' * 10 + 'Test'+'-' * 10)
    model_ft = torch.load('LFW_noface/model/efficientnet-b0(07).pth') #可单独使用测试函数
    # model_ft.load_state_dict(best_model_wts)
    criterion = nn.CrossEntropyLoss().cuda()
    test_model(model_ft, criterion,a = 0.5)
