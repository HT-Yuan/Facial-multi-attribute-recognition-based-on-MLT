...
from efficientnet.order_adience import EfficientNet
from efficientnet.order_adience import MTLLoss
import os
...
def train_model(model_ft, criterion, mloss, optimizer, lr_scheduler, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        for data in dset_loaders['train']:
            ...
            # print(loss_age,loss_gender,loss)
            # print(mloss.log_var2s.cpu()[0].item())
            ...
            if count % 30 == 0 or output_gender.size()[0] < batch_size:
                print('Epoch:{}: loss_age:{:.3f}, loss_gender:{:.3f},loss:{:.3f}'.format(epoch, loss_age.item(),loss_gender.item(),loss.item()))
        ...
        print('loss_age: {:.4f} loss_gender: {:.4f} loss: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f}'.format(
            epoch_loss_age, epoch_loss_gender,epoch_loss,epoch_acc_age, epoch_acc_gender))
    ...
    model_out_path = save_dir + "/" + net_name + '('+'mlt'+').pth'
...
def test_model(model, criterion,mloss):
    ...
    #这里在终端输出每次模型的评价指标
    file.write('Loss_age: {:.4f}  Loss_gender: {:.4f}  Loss: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f} mseloss_age: {:.4f} '.format(running_loss_age/dset_sizes,running_loss_gender/dset_sizes,running_loss/dset_sizes,
                                            running_corrects_age.double() / dset_sizes,running_corrects_gender.double() / dset_sizes, mseloss_age /dset_sizes))
    print('Loss_age: {:.4f}  Loss_gender: {:.4f} Loss: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f} mseloss_age: {:.4f} '.format(running_loss_age/dset_sizes,running_loss_gender/dset_sizes,running_loss/dset_sizes,
                                            running_corrects_age.double() / dset_sizes,running_corrects_gender.double() / dset_sizes, mseloss_age /dset_sizes))
    # p\r\f\总数
...
if __name__ == "__main__":
    ...
    model_ft = torch.load('mydata/model/efficientnet-b0('+"mlt"+').pth') #可单独使用测试函数
    # model_ft.load_state_dict(best_model_wts)
