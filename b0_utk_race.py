...
class MyDataset(Dataset):#继承了Dataset子类
    ...
    def __getitem__(self, index):
        ...
        return (input_img,age,gender,race)#返回成对的数据
...
def train_model(model_ft, criterion, optimizer, lr_scheduler, num_epochs=50):
    # answer.txt用作存储训练结果
    file = open('answer_b0gender_utk.txt','w')
    since = time.time()
    # 最好的训练权重，初始值为预训练权重
    ...
    model_out_path = save_dir + "/" + net_name + '(bo_race).pth'
...
if __name__ == "__main__":
    ...
    # model_ft._fc = nn.Linear(num_ftrs,5)  # race
    ...
    model_ft = torch.load('UTK_Face/model/efficientnet-b0(bo_race).pth')  #可单独使用测试函数
