...
def train_model(model_ft, criterion, optimizer, lr_scheduler, num_epochs=50, a = 0.1):
    # answer.txt用作存储训练结果
    file = open(os.path.join(answer_dir,'answer_hard('+str(a)+').txt'),'w')
    since = time.time()
    # 最好的训练权重，初始值为预训练权重
...
if __name__ == "__main__":
    ...
    model_ft = torch.load('mydata_14/model/efficientnet-b0('+str(a_temp)+').pth') #可单独使用测试函数
    # model_ft.load_state_dict(best_model_wts)
