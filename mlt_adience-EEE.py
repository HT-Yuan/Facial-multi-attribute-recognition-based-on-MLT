...
from efficientnet.e_adience import EfficientNet,MTLLoss
...
def convert_gray_to_rgb(image): 
    print("image no color")
    if image.shape[0] == 1:  # 检查图像是否是单通道的灰度图像 
        print("image no color")
        image = image.repeat(3, 1, 1)  # 复制灰度通道以创建三通道的图像  
    return image 
...
def train_model(model_ft, criterion, mloss,optimizer, lr_scheduler, num_epochs=50):
    # answer.txt用作存储训练结果
    ...
    for epoch in range(num_epochs):
        ...
        for data in dset_loaders['train']:
            ...
            loss = mloss(loss_age.cuda(),loss_gender.cuda())
            # 预测值
            pro_age = pro_age > 0.5
        ...
        file.write(str(epoch) + '\t' + str(float(epoch_loss_age)) + '\t'+str(float(epoch_loss_gender)) + '\t'+str(float(epoch_loss)) + '\t'+
                    str(float(epoch_acc_age))+'\t'+str(float(epoch_acc_gender))+'\t'+str(float(mloss.log_var2s.cpu()[0].item()))+'\t'+str(float(mloss.log_var2s.cpu()[1].item()))+'\n')
...
def test_model(model, criterion,mloss):
    ...
    not_success_gender = []
    # 读取每批次数据
    dset_loaders, dset_sizes = loaddata(data_dir=data_dir, batch_size=32, set_name='test', shuffle=False)
    for data in dset_loaders['test']:
        ...
        #读取的输出值
        output_age, output_gender,pro_age = model(inputs)
        # 消融实验
    ...
    #这里在终端输出每次模型的评价指标
    # file.write('Loss_age: {:.4f}  Loss_gender: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f} mseloss_age: {:.4f} maeloss_age: {:.4f}'.format(running_loss_age/dset_sizes,running_loss_gender/dset_sizes,
    ...
    print('Loss_age: {:.4f}  Loss_gender: {:.4f} Loss: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f} mseloss_age: {:.4f} maeloss_age: {:.4f}'.format(running_loss_age/dset_sizes,running_loss_gender/dset_sizes,running_loss/dset_sizes,
                                            running_corrects_age.double() / dset_sizes,running_corrects_gender.double() / dset_sizes, mseloss_age /dset_sizes,maeloss_age /dset_sizes))
    # p\r\f\总数
    ...
    plot_matrix(outLabel_age,outPre_age,[1,2,3,4,5,6,7],outLabel_gender,1, ["4-6","8-12","15-20","25-32","38-43","48-53","60-100"],"mlt_girl")
    # plot_matrix(outLabel_age,outPre_age,[1,2,3,4,5,6,7],outLabel_gender,2, ["4-6","8-12","15-20","25-32","38-43","48-53","60-100"],"mlt_boy")
...
def add_dict(state_dict):
    ...
    state_dict['_gender.fc.weight'] = torch.randn(([3,1280]))
...
if __name__ == "__main__":
    ...
    # # # 损失函数
    # criterion = nn.CrossEntropyLoss().cuda()
    mloss = MTLLoss(task_nums = 2).cuda()
    ...
    model_ft = torch.load('mydata/model/efficientnet-b0('+model_name+').pth') #可单独使用测试函数
    # model_ft.load_state_dict(best_model_wts)
        
    criterion = nn.CrossEntropyLoss().cuda()
    test_model(model_ft, criterion,mloss)
