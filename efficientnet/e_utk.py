...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None):
        ...
        self._conv_head_gender = Conv2d(in_channels, out_channels, kernel_size=1, bias=False)
        self._conv_head_race = Conv2d(in_channels, out_channels, kernel_size=1, bias=False)
        self._bn1 = nn.BatchNorm2d(num_features=out_channels, momentum=bn_mom, eps=bn_eps)

        self._bn1_gender = nn.BatchNorm2d(num_features=out_channels, momentum=bn_mom, eps=bn_eps)
        self._bn1_race = nn.BatchNorm2d(num_features=out_channels, momentum=bn_mom, eps=bn_eps)
        ...
        self._swish = MemoryEfficientSwish()
        self._swish_gender = MemoryEfficientSwish()
        self._swish_race = MemoryEfficientSwish()

    def set_swish(self, memory_efficient=True):
        """Sets swish function as memory efficient (for training) or standard (for export)"""
        self._swish = MemoryEfficientSwish() if memory_efficient else Swish()
        self._swish_gender = MemoryEfficientSwish() if memory_efficient else Swish()
        self._swish_race = MemoryEfficientSwish() if memory_efficient else Swish()
    ...
    def extract_features(self, inputs):  # 公共特征学习 以及 age深层特征
        ...
        extract_race = base_common    
        for idx, _ in enumerate(self._gender_BLOCKS):
                      
            extract_gender = self._gender_BLOCKS[idx](extract_gender,drop_connect_rate=drop_connect_rate_list[idx+7])
            extract_race = self._race_BLOCKS[idx](extract_race, drop_connect_rate=drop_connect_rate_list[idx+7])
        
        # extract_gender = self._gender_BLOCKS[0](base_common,drop_connect_rate=drop_connect_rate_list[7])
        
        # extract_gender = self._blocks[7](base_common,drop_connect_rate=drop_connect_rate_list[7])
        # print(extract_gender.size())
        # extract_gender = self._blocks[8](extract_gender,drop_connect_rate=drop_connect_rate_list[8])
        # print(extract_gender.size())
        # Head
        x = self._swish(self._bn1(self._conv_head(x)))

        extract_gender = self._swish_gender(self._bn1_gender(self._conv_head_gender(extract_gender)))

        extract_race = self._swish_race(self._bn1_race(self._conv_head_race(extract_race)))
        

        return x, extract_gender,extract_race
        # return x, extract_gender
    ...
    def forward(self, inputs):
        ...
        return x_age,x_gender,x_race,pro_a
...
class MTLLoss(nn.Module):
    ...
    def forward(self, L1, L2,L3): # 输入两个损失函
        loss = 0
        for i in range(len(self.log_var2s)):
            ...
            if i == 0:
                ...
                if self.log_var2s[i] < 0 :
                    loss += torch.sum(pre * L3, axis=-1)
                else:
                    loss += torch.sum(pre * L3 + self.log_var2s[i]/2, axis=-1)
                # loss += torch.sum(pre * L2 + self.log_var2s[i]/2, axis=-1)
