...
from efficientnet.e_utk import EfficientNet,MTLLoss
import os
from torch.utils.data import Dataset
from PIL import Image
from matrix import map_age_values,plot_heatmap,age_sub
...
class MyDataset(Dataset):#继承了Dataset子类
    ...
    def __getitem__(self, index):
        ...
        return (input_img,age-1,gender,race,names)#返回成对的数据
...
class CustomTransform(object):  
    ...
    def __call__(self, image):  
        # 检查图像是否具有Alpha通道（RGBA格式） 
        
        if image.mode == 'RGBA':  
            print(image.mode) 
            # 如果具有Alpha通道，分离出RGB通道并忽略Alpha通道  
            rgb_image = image.split()[:3]  
            rgb_image = Image.merge('RGB', rgb_image)  
        elif image.mode == 'L':  
            print(image.mode) 
            # 如果是单通道的灰度图像，复制颜色通道为三通道  
...
def loaddata_utk(data_dir, batch_size, set_name, shuffle):
    data_transforms = {
        'train': CustomTransform(transforms.Compose([
            #转为灰度
        ...
        'test': CustomTransform(transforms.Compose([
...
#     print(F.log_softmax(logits, dim=2)[:, :, 0])
...
def cost_fn(logits, levels, imp=1):
    # print(torch.nn.functional.softmax(logits, dim=2)[:, :, 0])
    # print(F.log_softmax(logits, dim=2)[:, :, 0].size())
...
def train_model(model_ft, criterion, mloss, optimizer, lr_scheduler, num_epochs=80):
    # answer.txt用作存储训练结果
    ...
    for epoch in range(num_epochs):
        ...
        for data in dset_loaders['train']:
            ...
            label_age = torch.squeeze(label_age.type(torch.LongTensor))
            ...
            for i in range(label_age.size()[0]):
                levels.append([1]*int(label_age[i]) + [0]*int(116 - 1 - label_age[i])) 
            ...
            # 标签转换完毕
            # 将数据转换为gpu训练的格式 
            inputs, label_age,label_gender,label_race,l_agehot = Variable(inputs.cuda()),Variable(label_age.cuda()),Variable(label_gender.cuda()),Variable(label_race.cuda()),Variable(levels.cuda())
            ...
            loss_age = cost_fn(output_age, l_agehot)
            
 
            loss_gender = criterion(output_gender, label_gender)
            loss_race = criterion(output_race, label_race)

            loss = mloss(loss_age.cuda(),loss_gender.cuda(),loss_race.cuda())
            ...
            if count % 30 == 0 or output_age.size()[0] < batch_size:
                print('Epoch:{}:  loss_age:{:.3f} loss_gender:{:.3f} loss_race:{:.3f} loss:{:.3f}'.format(epoch, loss_age.item(),loss_gender.item(),loss_race.item(),loss.item()))
        ...
        print('loss_age: {:.4f} loss_gender: {:.4f} loss_race: {:.4f} loss: {:.4f} Acc_age: {:.4f} Acc_gender: {:.4f} Acc_race: {:.4f}'.format(
            epoch_loss_age, epoch_loss_gender,epoch_loss_race,epoch_loss, epoch_acc_age, epoch_acc_gender,epoch_acc_race))
        ...
        file.write(str(epoch) + '\t' + str(float(epoch_loss_age)) + '\t'+str(float(epoch_loss_gender)) + '\t'+ str(float(epoch_loss_race)) + '\t' + str(float(epoch_loss)) + '\t' + \
                    str(float(epoch_acc_age))+'\t'+str(float(epoch_acc_gender))+'\t'+str(float(epoch_acc_race))+'\t'+ str(float(mloss.log_var2s.cpu()[0].item()))+'\t'+ \
                                    str(float(mloss.log_var2s.cpu()[1].item())) + '\t' +  str(float(mloss.log_var2s.cpu()[2].item())) + '\n')
...
def test_model(model, criterion):
    ...
    file = open(os.path.join(answer_dir,model_name+'.txt'),'a')
    model.eval()
    # 损失值
    running_loss_age = 0.0
    running_loss_gender = 0.0
    ...
    for data in dset_loaders['test']:
        ...
        running_corrects_gender += torch.sum(preds_gender == label_gender.data)
        running_corrects_race += torch.sum(preds_race == label_race.data)
        preds_age_mapped = torch.tensor(list(map(map_age_values, preds_age))) 
        label_age_mapped = torch.tensor(list(map(map_age_values, label_age.data))) 


        running_corrects_age_group += torch.sum(label_age_mapped == preds_age_mapped)
    ...
    print('Loss_age: {:.4f}  Acc_age: {:.4f} Acc_group: {:.4f} mseloss_age: {:.4f} maeloss_age: {:.4f}'.format(running_loss_age/dset_sizes,
                                            running_corrects_age.double() / dset_sizes, running_corrects_age_group.double() / dset_sizes,mseloss_age /dset_sizes, maeloss_age/dset_sizes))
    ...
    #                                         running_corrects_race.double() / dset_sizes))
    # # p\r\f\总数
    # print(len(outPre_age),len(outLabel_age))
    pred_age = list(map(map_age_values, outPre_age))
    label_age = list(map(map_age_values, outLabel_age))
    ...
    precision_group, recall_group, fscore_group, support_group = score(label_age, pred_age)
    # age_sub(outLabel_age, outPre_age)
    print('fscore_group: {}'.format(fscore_group))
    # plot_heatmap(outPre_age,outLabel_age,outPre_gender,outLabel_gender,outPre_race,outLabel_race,"mlt_utk_age",0)
    # plot_heatmap(outPre_age,outLabel_age,outPre_gender,outLabel_gender,outPre_race,outLabel_race,"mlt_utk_gender",1)
    plot_heatmap(outPre_age,outLabel_age,outPre_gender,outLabel_gender,outPre_race,outLabel_race,"mlt_utk_all",3)
    # precision_a, recall_a, fscore_a, support_a = score(outLabel_age, outPre_age)
    # precision_g, recall_g, fscore_g, support_g = score(outLabel_gender, outPre_gender)
...
mbcv_dict_key = ['._expand_conv.weight', '._bn0.weight', '._bn0.bias', '._bn0.running_mean', \
                 '._bn0.running_var', '._bn0.num_batches_tracked', '._depthwise_conv.weight', '._bn1.weight', '._bn1.bias', \
                    '._bn1.running_mean', '._bn1.running_var', '._bn1.num_batches_tracked', '._se_reduce.weight', '._se_reduce.bias', \
                        '._se_expand.weight', '._se_expand.bias', '._project_conv.weight', '._bn2.weight', '._bn2.bias', '._bn2.running_mean', \
                            '._bn2.running_var', '._bn2.num_batches_tracked']
...
#     state_dict['_gender.fc.weight'] = torch.randn(([2,80]))
#     state_dict['_gender.fc.bias'] = torch.randn(([2]))

#     state_dict['_race.Conv2d.weight'] = torch.randn(([112, 112, 3, 3]))
#     state_dict['_race.Conv2d.bias'] = torch.randn(([112]))
#     state_dict['_race.BN.weight'] = torch.randn(([112]))
#     state_dict['_race.BN.bias'] = torch.randn(([112]))
#     state_dict['_race.BN.running_mean'] = torch.randn(([112]))
#     state_dict['_race.BN.running_var'] = torch.randn(([112])) 
#     state_dict['_race.fc.weight'] = torch.randn(([5,112]))
#     state_dict['_race.fc.bias'] = torch.randn(([5]))
#     for key in mbcv_dict_key:
#         state_dict["_gender_BLOCKS.0"+key] = state_dict["_blocks.7"+key]
#     for key in mbcv_dict_key:
#         state_dict["_race_BLOCKS.0"+key] = state_dict["_blocks.7"+key]
#     for key in mbcv_dict_key:
#         state_dict["_race_BLOCKS.1"+key] = state_dict["_blocks.8"+key]
#     return state_dict
...
def add_dict(state_dict):
    ...
    # state_dict['_gender.BN.running_mean'] = torch.randn(([80]))
    # state_dict['_gender.BN.running_var'] = torch.randn(([80])) 
    state_dict['_gender.fc.weight'] = torch.randn(([2,1280]))
    state_dict['_gender.fc.bias'] = torch.randn(([2]))
    state_dict['_race.fc.weight'] = torch.randn(([5,1280]))
    state_dict['_race.fc.bias'] = torch.randn(([5]))

    state_dict["_conv_head_gender.weight"] = state_dict["_conv_head.weight"]
    state_dict["_bn1_gender.weight"] = state_dict["_bn1.weight"]
    state_dict["_bn1_gender.bias"] = state_dict["_bn1.bias"]
    state_dict["_bn1_gender.running_mean"] = state_dict["_bn1.running_mean"]
    state_dict["_bn1_gender.running_var"] = state_dict["_bn1.running_var"]

    state_dict["_conv_head_race.weight"] = state_dict["_conv_head.weight"]
    state_dict["_bn1_race.weight"] = state_dict["_bn1.weight"]
    state_dict["_bn1_race.bias"] = state_dict["_bn1.bias"]
    state_dict["_bn1_race.running_mean"] = state_dict["_bn1.running_mean"]
    state_dict["_bn1_race.running_var"] = state_dict["_bn1.running_var"]
    ...
    for idx in range(7,16):
        ...
        for key in mbcv_dict_key:
            ...
            state_dict["_gender_BLOCKS." + str(idx-7)+key] = state_dict["_blocks."+ str(idx)+key]
            state_dict["_race_BLOCKS." + str(idx-7)+key] = state_dict["_blocks."+ str(idx)+key]
...
if __name__ == "__main__":
    ...
    # #年龄
    # # print(num_ftrs)
    # model_ft._fc = nn.Linear(num_ftrs,115*2)  # age
    ...
    # mloss = MTLLoss(task_nums = 3).cuda()
    # parameters = list(model_ft.parameters())
    # # # print(list(parameters))
    # parameters.append(*mloss.parameters())

    # # 使用SGD用作训练 同类型的优化方法有ADAM等
    # optimizer = optim.SGD(parameters, lr=lr,
    #                     momentum=momentum, weight_decay=0.0004)
    # # 调用训练函数
    # best_model_wts = train_model(model_ft, criterion, mloss, optimizer, exp_lr_scheduler, num_epochs=num_epochs)
